      ...
      <div className="p-6 max-w-lg">
        <div className="bg-emerald-900/20 border border-emerald-700/40 rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-5">
            <div className="w-10 h-10 rounded-full bg-emerald-900 flex items-center justify-center text-emerald-400 text-lg">✓</div>
            <div>
              <h2 className="text-base font-bold text-white">Şagird əlavə edildi</h2>
              <p className="text-xs text-emerald-400 mt-0.5">Giriş məlumatlarını indi saxlayın — sonra göstərilməyəcək</p>
            </div>
          </div>
          ...
          <div className="space-y-3 mb-6">
            {[
              ...
              <div key={key} className="bg-slate-900 rounded-xl px-4 py-3 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-xs text-slate-500 mb-0.5">{label}</p>
                  <p className={`text-sm font-mono text-white select-all truncate ${key === "pw" ? "font-bold text-emerald-300 tracking-wider" : ""}`}>
                    {value}
                  </p>
                </div>
                <button type="button" onClick={() => copy(value, key)}
                  className="shrink-0 text-xs text-slate-500 hover:text-slate-200 px-2.5 py-1 rounded-lg
          ...
          <button type="button"
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl
                       ...
                       transition-colors border border-slate-700/50 mb-3">
            🖨️ Kart çap et
          </button>
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => { setCreated(null); setForm({ name: "", email: "", password: "", class_name: form.class_name, group_name: form.group_name }); }}
              className="flex-1 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium transition-colors">
              + Növbəti şagird
            </button>
